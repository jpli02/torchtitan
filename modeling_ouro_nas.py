...
def _build_router_hf(dim: int) -> nn.Module:
    """Build the early-exit gate from OURO_ROUTER_SPEC, for NAS candidates.

    Mirrors torchtitan/models/ouro/model.py::build_router so that an
    architecture found by the NAS can also be SERVED. The HF path is what
    ouro_openai_server.py loads, and it previously hard-coded nn.Linear(dim, 1),
    which meant any non-linear router could be trained but never evaluated on
    terminal-bench.

    Backwards compatible by construction: with no OURO_ROUTER_SPEC set (or an
    empty/failed parse) this returns exactly nn.Linear(dim, 1), so every
    existing checkpoint loads bit-identically.
    """
    ...
    if raw:
        try:
            ...
            logging.warning("OURO_ROUTER_SPEC is not valid JSON; using linear gate")
...
class OuroModel(OuroPreTrainedModel):
    def __init__(self, config: OuroConfig):
        ...
        self.early_exit_gate = _build_router_hf(config.hidden_size)
